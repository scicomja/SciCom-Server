...
const { badRequest, escapeForRegex } = require("../utils")

const { validateParameters, constructQuery } = require("./validator/user")

const { unauthorized, notFound } = require("../utils")
...
const optionalFields = _.differenceWith(
...
const upload = multer({
	...
	fileFilter: (req, file, cb) => {
		...
		if (file.fieldname === "avatar" && !imageTypes.test(file.mimetype)) {
			return cb(new Error("Only images are allowed"))
		}
		if (file.fieldname === "CV" && !/pdf/.test(file.mimetype)) {
...
const UserSchema = new mongoose.Schema(rawSchema).plugin(
...
UserSchema.pre("remove", async function(next) {
	...
	try {
		if (isPolitician) {
			...
			removeProjects.forEach(async project => {
